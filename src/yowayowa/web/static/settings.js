(() => {
  ...
  function enabledIds() {
    ...
    const meta = readMeta();
    let ids = Array.isArray(saved) ? saved.filter(id => providers.some(p => p.id === id)) : ['openrouter'].filter(id => providers.some(p => p.id === id));
    if (meta.providerId && meta.providerId !== 'server' && providers.some(p => p.id === meta.providerId) && !ids.includes(meta.providerId)) ids.push(meta.providerId);
    return ids;
  }
  ...
  function save() {
    ...
    const meta = { providerId, adapter: provider?.adapter || 'openai_compatible', model: modelInput.value.trim(), baseUrl: baseInput.value.trim(), contextualEnabled: contextToggle.checked };
    try {
  ...
  function rebuildProviderSelect(preferred = null) {
    ...
    providerSelect.innerHTML = '<option value="server">Server configuration</option>' + providers.filter(item => enabledProviders.includes(item.id)).map(item => `<option value="${escapeHtml(item.id)}">${escapeHtml(item.label)}</option>`).join('');
  ...
  function renderProviderCatalog() {
    ...
    const active = providers.filter(item => enabledProviders.includes(item.id));
  ...
  function renderProviderManager() {
    ...
    const manager = document.createElement('details'); manager.className = 'provider-manager';
    manager.innerHTML = `<summary><span>${ja ? '利用するプロバイダを管理' : 'Manage enabled providers'}</span><span class="pill">${enabledProviders.length}/${providers.length}</span></summary><div class="provider-toggle-grid">${providers.map(item => `<label class="provider-toggle"><input type="checkbox" value="${escapeHtml(item.id)}" ${enabledProviders.includes(item.id) ? 'checked' : ''}><span>${escapeHtml(item.label)}</span></label>`).join('')}</div>`;
    catalog.insertAdjacentElement('beforebegin', manager);
    manager.addEventListener('change', () => {
      enabledProviders = [...manager.querySelectorAll('input:checked')].map(input => input.value);
  ...
  function installModelFilter(target) {
    ...
    const apply = () => {
      const buttons = [...target.querySelectorAll('.model-result')]; const q = bar.querySelector('input').value.trim().toLowerCase(); let shown = 0;
      buttons.forEach(button => { const hit = !q || button.textContent.toLowerCase().includes(q); button.hidden = !hit; if (hit) shown += 1; });
  ...
  async function fetchModels() {
    const provider = selectedProvider(); const target = document.querySelector('#model-results'); if (!provider) return;
    ...
    try {
      const data = await api('/v1/ai/models', { method: 'POST', body: JSON.stringify({ provider: { provider: provider.adapter, model: modelInput.value.trim(), api_key: apiKey, base_url: baseInput.value.trim() || provider.base_url }, limit: 1000 }) });
      target.hidden = false;
      target.innerHTML = data.models.length ? data.models.map(model => `<button class="model-result" type="button" data-model="${escapeHtml(model)}">${escapeHtml(model)}</button>`).join('') : '<div class="list-state muted">No models returned.</div>';
      target.querySelectorAll('.model-result').forEach(button => button.addEventListener('click', () => { modelInput.value = button.dataset.model; save(); }));
      installModelFilter(target); status.textContent = `${data.models.length} models`;
  ...
  async function sha256Base64Url(text) { const digest = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(text)); return bytesToBase64Url(new Uint8Array(digest)); }

  async function startOpenRouterOAuth() {
    const bytes = new Uint8Array(64); crypto.getRandomValues(bytes); const verifier = bytesToBase64Url(bytes); const challenge = await sha256Base64Url(verifier);
    const callback = new URL('/settings', location.origin); const lang = new URLSearchParams(location.search).get('lang'); if (lang) callback.searchParams.set('lang',lang);
    try { sessionStorage.setItem(PKCE_KEY, JSON.stringify({ verifier, callback: callback.toString() })); } catch (_) {}
    const auth = new URL('https://openrouter.ai/auth'); auth.searchParams.set('callback_url',callback.toString()); auth.searchParams.set('code_challenge',challenge); auth.searchParams.set('code_challenge_method','S256'); location.assign(auth.toString());
  }

  async function finishOpenRouterOAuth() {
    const code = new URLSearchParams(location.search).get('code'); if (!code) return false;
    let pkce={}; try { pkce=JSON.parse(sessionStorage.getItem(PKCE_KEY)||'{}'); } catch (_) {}
    if (!pkce.verifier) { status.textContent='OpenRouter OAuth verifier is missing.'; return true; }
    status.textContent='OpenRouter OAuth…';
    try {
      const response=await fetch('https://openrouter.ai/api/v1/auth/keys',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code,code_verifier:pkce.verifier,code_challenge_method:'S256'})});
      if(!response.ok) throw new Error(`OpenRouter OAuth HTTP ${response.status}`); const data=await response.json(); if(!data.key) throw new Error('OpenRouter did not return an API key.');
      sessionStorage.setItem(KEY_KEY,data.key); keyInput.value=data.key; if(!enabledProviders.includes('openrouter')) { enabledProviders.push('openrouter'); saveEnabled(); renderProviderManager(); renderProviderCatalog(); } rebuildProviderSelect('openrouter'); syncProvider(true); save(); sessionStorage.removeItem(PKCE_KEY);
      const clean=new URL(location.href); clean.searchParams.delete('code'); history.replaceState(null,'',clean.toString()); status.textContent='OpenRouter connected.';
    } catch(error) { status.textContent=error.message; }
    return true;
  }
  ...
  Promise.all([loadProviders(),loadDataStatus()]).then(()=>finishOpenRouterOAuth()).catch(error=>{status.textContent=error.message;});
})();
